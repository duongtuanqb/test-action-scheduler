...
class McdScrSgSpider(scrapy.Spider):
    ...
    def start_requests(self):
        try:
            url = "https://www.xe.com/currencyconverter/convert/?Amount=1&From=SGD&To=VND"
    ...
    def parse_fx(self, response):
        ...
        exchange_rate = re.sub(
            r",", "",
            response.xpath("//span[contains(@class,'faded-digits')]/..//text()").get()
        )
        faded_digits = response.xpath("//span[contains(@class,'faded-digits')]/text()").get()
        exchange_rate = float(exchange_rate + faded_digits)

        print(
            f"\n1 SGD = {exchange_rate} VND (1 VND = {1 / exchange_rate} SGD) "
            ...
            try:
                ...
                for submenu in submenus:
                    ...
                    for food in products:
                        ...
                        product["Price (VND)"] = round((float(product["Price (SGD)"]) * exchange_rate), 2)
                ...
                print(f"\n\n---------\n Error occurred while parsing product data.")
                print(f"{traceback.format_exc()}\n---------\n")
